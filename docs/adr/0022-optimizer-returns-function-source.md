...
4. gofmt the new declaration in isolation, then splice it over the old one's byte range. The old doc
   ...
   has one.
5. Add any `imports` paths the file does not already have, into its existing import block or a new
   one, formatted with `go/format` — which is what sorts import groups — so the result is exactly
   what a careful hand-written patch would have produced.
...
The import-insertion path rebuilds the whole import block from scratch (existing specs plus the
missing ones, gofmt-sorted) rather than editing the original text in place; a file whose imports were
split into blank-line-separated groups (standard library, then third-party) loses that grouping,
though not sortedness or correctness — `go/format` still produces a single valid group, and the
target still builds. Splitting into groups again is left for a future patch if it is measured to
matter.
