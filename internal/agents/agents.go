...
FUNCTION_SET TARGET: when target.kind is "function_set", the remedy spans target.function and the callers listed in target.callers, and a patch that leaves every caller unchanged is rejected before it is built. Return function_sources instead of function_source: a list with one complete declaration per function you change: the callee, any new helper beside it (new functions may only go in the callee's own file), and each caller you switch, each copied from its source_excerpt and edited in place. Switch as many listed callers as the remedy applies to; do not change any function outside target.function, target.callers and your new helper. imports works as above, covering every file you touch.
