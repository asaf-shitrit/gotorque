...
func replaceImportBlock(fset *token.FileSet, data []byte, decl *ast.GenDecl, missing []string) ([]byte, error) {
	...
	block, err := importBlockWith(fset, data, decl, missing)
	if err != nil {
		return nil, err
	}
	var buf bytes.Buffer
...
// importBlockWith returns the import declaration with missing added to its
// first group, the one a Go file keeps the standard library in, and every
// other line as it was. gofmt sorts within a group and never across the blank
// line between groups, so a file's standard-library and third-party groups
// survive; rebuilding the block from its specs merged them into one, a
// reformat of the target's code no remedy asked for. A single unparenthesized
// import has no groups to keep and becomes a block.
func importBlockWith(fset *token.FileSet, data []byte, decl *ast.GenDecl, missing []string) (string, error) {
	var added strings.Builder
	for _, imp := range missing {
		added.WriteString("\t" + strconv.Quote(imp) + "\n")
	}
	start := fset.Position(decl.Pos()).Offset
	end := fset.Position(decl.End()).Offset
	text := string(data[start:end])
	if !decl.Lparen.IsValid() {
		spec := strings.TrimSpace(strings.TrimPrefix(text, "import"))
		text = "import (\n" + added.String() + "\t" + spec + "\n)"
	} else {
		open := fset.Position(decl.Lparen).Offset - start + 1
		text = text[:open] + "\n" + added.String() + strings.TrimLeft(text[open:], "\n")
	}
	return formatImportDecl(text)
}

// formatImportDecl gofmts one import declaration on its own.
func formatImportDecl(text string) (string, error) {
	...
	return string(formatted[fset.Position(decl.Pos()).Offset:fset.Position(decl.End()).Offset]), nil
}
...
func formatImportBlock(specs []string) (string, error) {
	...
	b.WriteString(")")
	return formatImportDecl(b.String())
}
