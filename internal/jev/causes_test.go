...
func TestFlaggedKeepsAtMostTwoCausesAboveTheThreshold(t *testing.T) {
	scores := []Score{{Cause: CauseUnbufferedIO, Z: 3}, {Cause: CauseAlloc, Z: 1}, {Cause: CausePrealloc, Z: 0.8}, {Cause: CauseRedundant, Z: 0.1}}
	...
	if got := Flagged([]Score{{Cause: CauseAlloc, Z: FlagThreshold - 0.01}}); len(got) != 0 {
